...
def to_processing_response(result: "ProcessingResult") -> dict:
    """Map a BANK74 ProcessingResult (prompt 04) to the JSON response body."""
    body = {
        "transaction_id": result.transaction_id,
        "success": result.success,
        "status": result.error_code.value,
        ...
        "updated_balance": None if result.updated_balance is None else str(result.updated_balance),
        "high_risk_accounts": list(result.high_risk_accounts),
        "message": result.message,
