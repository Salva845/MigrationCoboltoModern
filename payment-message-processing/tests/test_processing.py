...
Covers prompt 04: amount-threshold validation (BR-0011/BR-0012), fixed 15.00 fee
(BR-0013), insufficient-funds check (BR-0014), atomic debit (BR-0015), high-risk
account scan (BR-0018), type-6 timeout simulation (BR-0017), and the removal of
the type-4 authorization denial (BR-0016). Monetary assertions use Decimal.
"""
...
def test_tc0021_amount_exceeds_maximum_is_rejected():
    proc = build_processor([acc(balance="2000000.00")])
    result = proc.process("T21", message(amount="1000000.01"))

    assert result.success is False
    assert result.error_code is ErrorCode.INVALID_FORMAT
    assert result.state == State.AUDIT == 40
    assert balance_of(proc) == Decimal("2000000.00")  # untouched
...
def test_minimum_is_validated_before_maximum():
    # A sub-minimum amount fails on BR-0011 even though config bounds both apply.
    proc = build_processor([acc(balance="1000.00")])
...
def test_tc0027_equivalence_with_legacy_oracle():
    ...
    body = to_processing_response(result)
...
def test_error_code_to_http_status_mapping():
    ...
    assert status_for(ErrorCode.ACCOUNT_NOT_FOUND) == 404
    assert status_for(ErrorCode.TIMEOUT) == 504


def test_controller_maps_results_to_http_status():
    repo = InMemoryAccountRepository([acc(balance="115.00")])
    ctrl = TransactionProcessingController(account_repository=repo)

    ok = ctrl.process({"transaction_id": "P1", **message(amount="100.00")})
    assert ok.status_code == 200
    assert ok.body["success"] is True
    assert ok.body["updated_balance"] == "0.00"

    insuff = ctrl.process({"transaction_id": "P2", **message(amount="100.00")})
    assert insuff.status_code == 409
    assert insuff.body["error_code"] == ErrorCode.INSUFF_FUNDS.value


def test_controller_rejects_missing_transaction_id():
    ctrl = TransactionProcessingController(account_repository=InMemoryAccountRepository([acc()]))
    resp = ctrl.process(message(amount="100.00"))  # no transaction_id
    assert resp.status_code == 400
    assert resp.body["error_code"] == ErrorCode.INVALID_FORMAT.value


# --------------------------------------------------------------------------- #
# HTTP round trip — wired into the service request handler (POST /transactions/process)
# --------------------------------------------------------------------------- #
def test_http_post_process_end_to_end():
    repo = InMemoryAccountRepository([acc(balance="1000.00")])
    validate_ctrl = TransactionValidationController(account_repository=repo)
    process_ctrl = TransactionProcessingController(account_repository=repo)
    server = build_server(
        validate_ctrl, host="127.0.0.1", port=0, processing_controller=process_ctrl
    )
    port = server.server_address[1]
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    try:
        payload = json.dumps({"transaction_id": "HTTP74", **message(amount="100.00")}).encode()
        req = urllib.request.Request(
            f"http://127.0.0.1:{port}/transactions/process",
            data=payload,
        ...
        assert balance_of(process_ctrl.processor) == Decimal("885.00")
    finally:
        server.shutdown()
        server.server_close()


def test_http_process_route_absent_without_controller_is_404():
    repo = InMemoryAccountRepository([acc()])
    server = build_server(TransactionValidationController(account_repository=repo), port=0)
    port = server.server_address[1]
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    try:
        req = urllib.request.Request(
            f"http://127.0.0.1:{port}/transactions/process", data=b"{}", method="POST"
        )
