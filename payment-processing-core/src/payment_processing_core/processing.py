...
Wave 1 / prompt 05: "Process a financial transaction with validation, fee
calculation, and balance update" in **payment-processing-core**. A self-contained
slice that parses an ISO 8583 message, validates the amount against policy
thresholds (BR-0011/BR-0012), adds a fixed fee (BR-0013), checks sufficiency
(BR-0014), debits the account atomically (BR-0015), simulates a credit-state
timeout for type 6 (BR-0017) and scans every account for high-risk balances
during the logging state (BR-0018).

This is a *different* functional slice from the BANK85 compliance/fraud flow in
``validator.py``: there is no VAT, the fee is a flat 15.00 and the high-risk
threshold is 90,000.00. The shared account store, ISO parser, state machine and
error codes are reused from this same package (DATABASE / ISO-PARSER / MESSAGES
copybooks). The HTTP transport wrapper lives in ``payment-message-processing``
(prompt 04) and the cross-service wiring is handled by sibling prompt 06.
...
class ProcessingConfig:
    """Numeric policy for the BANK74 slice (SECURITY-RULES.CPY replacement).
...
class ProcessingResult:
    ...
    def to_dict(self) -> dict:
        """Serialise to a JSON-ready dict (monetary values as fixed-point strings).

        The shape mirrors the legacy transaction record so an equivalence pass can
        compare it byte-for-byte against the COBOL oracle (TC-0027).
        """
        return {
            ...
            "status": self.error_code.value,
            "state": int(self.state),
            "original_amount": str(self.original_amount),
            "fee": str(self.fee),
            "total_debit": str(self.total_debit),
            "updated_balance": None if self.updated_balance is None else str(self.updated_balance),
...
class TransactionProcessor:
    ...
    def br0018_high_risk_scan(self) -> list[str]:
        ...
        The threshold is a hard boundary: a balance of exactly the threshold is
        not flagged (``>`` not ``>=``).
        """
        flagged = [
            a.account_id
